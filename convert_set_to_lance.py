...
def split_raw_to_fixed_length_samples(
    ...
    """
    把连续 EEG Raw 切成固定长度 sample，并保留 metadata。
    ...
    while start + clip_len <= total_points:
        ...
        samples.append({
            ...
            "clip_idx": clip_idx,
            "clip_start_sec": float(start / sfreq),
        })
...
def load_multiple_eeglab_sets_as_samples(
    ...
    for set_path in set_paths:
        ...
        try:
            ...
            print(f"[WARNING] Failed to load SET file, skip: {set_path}")
            print(f"[WARNING] Error: {repr(e)}")
            failed_files.append(set_path)
            continue

    print("\n===== Load Summary =====")
    print(f"successful clips: {len(all_samples)}")
    print(f"failed files: {len(failed_files)}")

    if len(failed_files) > 0:
        failed_log = "failed_set_files.txt"
...
def collect_set_paths(args, cfg):
    """
    收集 .set 文件路径。
    ...
    优先级:
        1. 命令行 --set_paths
        2. 命令行 --set_dir
        3. cfg.data.set_paths
    """
    set_paths = []

    if args.set_paths:
        set_paths.extend(args.set_paths)

    if args.set_dir:
        set_dir = Path(args.set_dir)

        found = []
        for x in set_dir.rglob("*.set"):
            # 跳过 macOS 生成的 AppleDouble 隐藏文件
        ...
        set_paths.extend(sorted(found))

    if len(set_paths) == 0 and hasattr(cfg.data, "set_paths"):
        set_paths.extend(cfg.data.set_paths)

    # 再过滤一遍，防止 cfg.data.set_paths 里也有 ._ 文件
    clean_paths = []
    for p in set_paths:
        name = Path(p).name
    ...
    set_paths = sorted(set(clean_paths))
...
def main():
    ...
    parser.add_argument(
        "--output",
        default="data/pretrain_processed_set.lance",
        help="输出 Lance dataset 路径",
    )
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=None,
        help="一个或多个 EEGLAB .set 文件路径",
    )

    parser.add_argument(
        "--set_dir",
        default=None,
        help="包含 .set 文件的文件夹，会递归搜索 *.set",
    )
    ...
    set_paths = collect_set_paths(args, cfg)

    print("===== SET Files =====")
    for p in set_paths[:20]:
        print(p)

    if len(set_paths) > 20:
        print(f"... and {len(set_paths) - 20} more")

    if len(set_paths) == 0:
        raise ValueError(
            "No .set files found. Please pass --set_paths, --set_dir, "
            "or define cfg.data.set_paths."
        )
